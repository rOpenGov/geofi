...
```{r muni_pop_map1, fig.height = 7, fig.width = 4}
...
```{r muni_pop_map2, fig.height = 7, fig.width = 4}
get_municipality_pop(year = 2020) %>%  
...
```{r muni_pop_map3, fig.height = 7, fig.width = 4}
get_municipality_pop(year = 2020) %>%  
...
Total population (`vaesto`), number of men (`miehet`) and women (`naiset`), under 15 year olds (`ika_0_14`), 15-64 year olds (`ika_15_64`), and aged over 65 (`ika_65_`). Only the number of population is reported for grids of under 10 inhabitants. See [Population grid data](https://www.stat.fi/org/avoindata/paikkatietoaineistot/vaestoruutuaineisto_5km_en.html).
...
libs <- c("sotkanet","geofacet","ggplot2")
if (check_namespaces(pkgs = libs)) {
  # Let pull population data from Statistics Finland
  library(sotkanet)
  sotkadata <- GetDataSotkanet(indicators = 127, years = 2010:2020) %>% 
