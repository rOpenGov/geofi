...
This vignettes provides few examples on how to join attribute data from common sources of attribute data. Here we are using data from three different sources of which two are from [Statistics Finland PxWeb-api](https://pxdata.stat.fi/PXWeb/pxweb/en/), namely [municipality key figures](https://pxdata.stat.fi/PXWeb/pxweb/en/Kuntien_avainluvut/) and [Paavo (Open data by postal code area)](https://pxdata.stat.fi/PXWeb/pxweb/en/Postinumeroalueittainen_avoin_tieto/). Third is [Covid-19 tests and confirmed cases in Finland](https://thl.fi/fi/tilastot-ja-data/aineistot-ja-palvelut/avoin-data/varmistetut-koronatapaukset-suomessa-covid-19-).
...
In this first example we join municipality level data from Statistics Finland [municipality key figures](https://pxdata.stat.fi/PXWeb/pxweb/en/Kuntien_avainluvut/)
...
library(sotkanet)
sotkadata <- GetDataSotkanet(indicators = 7, years = 2010:2020) %>% 
  # subset to container only current municipalities
  filter(region %in% geofi::municipality_key_2023$municipality_code) %>% 
  rename(municipality_code = region)
} else {
