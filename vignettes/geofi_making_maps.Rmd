...
check_namespaces <- function(pkgs){
  pkglst <- list()
  for (i in seq_along(pkgs)) pkglst[[i]] <- requireNamespace(pkgs[i], 
                                                             quietly = TRUE)
  return(all(unlist(pkglst)))
}
```
...
```{r municipality_map, fig.width = 5}
library(geofi)
...
[`tmap`](https://mtennekes.github.io/tmap/) is a versatile library for creating static thematic maps in R. It supports `sf`-class objects and is fully compatible with geospatial data available through `geofi`.
...
```{r uusimaa, fig.width=8, fig.height=4}
libs <- c("ggplot2")
if (check_namespaces(pkgs = libs)) {
library(dplyr)
...
```{r uusimaa_repel, fig.width=8, fig.height=4}
libs <- c("ggplot2")
if (check_namespaces(pkgs = libs)) {
ggplot() + 
  theme_light() +
...
```{r patchwork, fig.width = 8, fig.height=10}
