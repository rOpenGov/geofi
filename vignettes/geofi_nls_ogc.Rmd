---
title: "Accessing Spatial Data from Maastotietokanta and Geographic Names with geofi"
author: "Markus Kainu"
date: "`r Sys.Date()`"
...
Search for "kainu" within a bounding box:
...
Both `ogc_get_maastotietokanta()` and `ogc_get_nimisto()` support `custom_params` for advanced API queries. For example:

```{r}
cemeteries_filtered <- ogc_get_maastotietokanta(
  collection = "hautausmaa",
  custom_params = "filter=attribute='value'"
)
```

Consult the [NLS OGC API documentation](https://www.maanmittauslaitos.fi/en/rajapinnat) for valid parameters.
