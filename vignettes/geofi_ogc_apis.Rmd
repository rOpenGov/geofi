...
# 
# lappi <- ogc_get_statfi_area(tessellation = "maakunta", 
#                              crs = 4326) |>
...
# ggplot(muni) + geom_sf()
# 
# all <- ogc_get_statfi_area_pop(year = 2022,
#                               bbox = lappi_bbox,
#                               crs = 3067
#                               )
# dim(all)
...
#                               crs = 4326)
# ggplot(data = ff[grepl("^kunta", ff$areaStatisticalUnit_inspireId_localId),], 
#        aes(fill = female_percentage)) + geom_sf()
# 
# 
...
# dplyr::as_tibble(res) %>% print(n = 200)
# 
# ff <- ogc_get_maastotietokanta(collection = "suo", bbox = helsinki_bbox)
# dim(ff)
# ggplot(ff) + geom_sf()
# mapview::mapview(ff)
# 
# 
# fff <- ogc_get_nimisto(search_string = "kainu")
# mapview::mapview(fff)
