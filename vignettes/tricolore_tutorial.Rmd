...
```{r municipalities_with_data, eval = FALSE}
...
finland_popu2017 <- left_join(dat, municipality_key_2017, by = c("name" = "name_fi"))
```
...
```{r plot_simplemap1, fig.height = 8, dpi=100, eval = FALSE}
...
```{r plot_simplemap2, fig.height = 8, dpi=100, eval = FALSE}
plot_pop <- ggplot(finland_popu2017) +
  geom_sf(aes(fill = popu_rgb, geometry = geom), size = .1) +
  scale_fill_identity() 
plot_pop
...
plot_pop <- ggplot(finland_popu2017) +
  ...
  labs(title = "Finnish population by municipalites",
       ...
       caption = "Data by Statistics Finland")
plot_pop
```
